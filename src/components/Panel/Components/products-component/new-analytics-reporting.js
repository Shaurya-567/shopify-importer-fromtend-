...
class Demo_analytics_reporting extends Component {
    ...
    getActiveRecurrying() {
        let plan_to_be_end = "";
        requests
            .getRequest("plan/plan/getActive", undefined, false, true)
            .then(response => {
                if (response.success) {
                    ...
                    if (
                        new Date(
                            new Date(add_on_date).setMonth(
                                add_on_date.getMonth() + difference + 1
                            )
                        ) <= current_date
                    ) {
                        plan_to_be_end = new Date(
                            new Date(add_on_date).setMonth(
                                add_on_date.getMonth() + difference + 2
                            )
                        );
                    } else {
                        plan_to_be_end = new Date(
    ...
    getServiceCredits() {
        requests
            .getRequest(
                "shopifygql/payment/getCreditsSettings",
                undefined,
                false,
                true
            )
            .then(response => {
                if (response.success) {
                    if (
                        response.data.available_credits === 0 &&
                        response.data.used_credits === 0
                    ) {
                        this.setState({
                            ...
                            datasets: [
                                {
                                    data: [
                                        ...
                                        response.data.used_credits
                                    ],
                                    backgroundColor: this.state.backgroundColor,
                                    hoverBackgroundColor: this.state
                                        .hoverBackgroundColor
                                }
                            ],
    ...
    getYAxisImporter(
        ...
        requests
            .postRequest(
                "frontend/app/getImportedProductCount",
            ...
            .then(data => {
                if (
                    data.success &&
                    (data["data"]["amazonaffiliate"] !== 0 ||
                        data["data"]["amazonimporter"] !== 0 ||
                        data["data"]["ebayimporter"] !== 0 ||
                        data["data"]["etsyimporter"] !== 0 ||
                        data["data"]["walmartimporter"] !== 0 ||
                        data["data"]["wishimporter"] !== 0 ||
                        data["data"]["ebayaffiliate"] !== 0 ||
                        data["data"]["fileimporter"] !== 0 ||
                        data["data"]["aliexpress"] !== 0)
                ) {
                    importer_data_rec = data.data;
                    ...
                    Object.keys(importer_data_rec).map(importer_recieved_mp => {
                        for (
                            let i = 0;
                            i < importer_marketplace_array.length;
                            i++
                        ) {
                            Object.keys(entire_data_importer).map(
                                master_key => {
                                    if (
                                        importer_marketplace_array[i] ===
                                            entire_data_importer[master_key][
                                                "marketplace"
                                            ] &&
                                        ...
                                        if (
                                            data.data[importer_recieved_mp] > 0
                                        ) {
                                            total_products_importer.push(
                                                importer_data_rec[
                                                    importer_recieved_mp
                    ...
                    label_mp_array = label_mp_array.map(e1 =>
                        capitalizeWord(e1)
                    );
                    ...
                    Object.keys(data1.data).forEach(e => {
                        if (data1.data[e] !== undefined) {
                            if (data1.data[e]["_id"] === null) {
                                ...
                                uploader.push(
                                    capitalizeWord(data1.data[e]["_id"])
                                );
                                uploaderarray.push(data1.data[e]["count"]);
                                show = true;
                            }
    ...
    to_final_render_Doughnut() {
        ...
        let temp_products = this.state
            .no_getProductsUploadedData_and_ImportedData;
        ...
        for (let i = 0; i < 3; i++) {
                    ...
                    >
                        ...
                        >
                            <Stack distribution="center">
                                <img
                                    className="img-fluid "
                                    src={require("../../../../assets/img/222x176.png")}
                                />
                            </Stack>
                        </Card>
                    </div>
            ...
            arr.push(
                <div
                    className="col-sm-12 col-md-12 col-lg-4 no-gutters"
                    key={yourVariable}
                    onClick={() => {
                        this.redirect(this.state.to_redirect[i]);
                    }}
                >
                    {this.state.skeleton[i] ? (
                        <Skeleton />
                    ) : (
    ...
    recurring_plan() {
        if (this.state.Recurrying === true) {
            return this.state.recurring_planskeleton ? (
                <Skeleton case="body" />
            ) : (
                <Card>
                    <ResourceList
            ...
            return this.state.recurring_planskeleton ? (
                ...
                <Layout>
                    ...
                    <Layout.Section>
                        <img
                            className="img-fluid"
                            src={require("../../../../assets/img/320x260.png")}
                        />
                    </Layout.Section>
                </Layout>
            );
    ...
    render() {
        ...
        for (let i = 0; i < this.state.content_data.datanews.length; i++) {
            rows.push({
                url: this.state.content_data.datanews[i]["label"][
                    "content_link"
                ],
                name: this.state.content_data.datanews[i]["label"]["title"],
                description: this.state.content_data.datanews[i]["label"][
                    "description"
                ],
                media: (
                    <Thumbnail
                        source={
                            this.state.content_data.datanews[i]["label"][
                                "image_url"
                            ]
                        }
                        alt="News Logo"
        ...
        for (let i = 0; i < this.state.content_data.datablog.length; i++) {
            rows_blog.push({
                url: this.state.content_data.datablog[i]["label"][
                    "content_link"
                ],
                name: this.state.content_data.datablog[i]["label"]["title"],
                description: this.state.content_data.datablog[i]["label"][
                    "description"
                ],
                media: (
                    <Thumbnail
                        source={
                            this.state.content_data.datablog[i]["label"][
                                "image_url"
                            ]
                        }
                        alt="News Logo"
                                ...
                                <ResourceList
                                    items={[
                                        ...
                                        },
                                        {
                                            ...
                                            media: (
                                                <Thumbnail
                                                    source={require("../../../../assets/img/CNyDx+T0lu8CEAE=.png")}
                                        ...
                                        {
                                            url:
                                                "https://apps.shopify.com/google-express-integration?utm_source=importer_notifiation",
                                            name:
                                            ...
                                            media: (
                                                <Thumbnail
                                                    source={require("../../../../assets/img/google_feed.jpg")}
                        ...
                        <Layout.Section oneThird>
                            <Card title="News">
                                <Card.Section>
                                    <Stack distribution="center">
                                        <img
                                            className="img-fluid pt-5 mt-2"
                                            src={require("../../../../assets/img/data_nahi.png")}
                                        />
                                    </Stack>
                                </Card.Section>
                            </Card>
                                ...
                                actions={[
                                    {
                                        content: "See all",
                                        url:
                                            "https://cedcommerce.com/blog/?s=omni+importer",
                                        external: true
                                    }
                                ]}
                            >
        ...
        return (
            <div>
                <div className="justify-content-center fleximagedashborad">
                    ...
                    <div
                        className="text-center markmargingimg"
                        onClick={() => {
                            this.redirect("/panel/help/report");
                        }}
                    ><Heading >Contact US</Heading>
                        <img
                            ...
                            alt="Store Development"
                            /*height="650" width="650"*/
                        />
                    </div>
                    <div class="vl" />
                    <div className="justify-content-center text-left imagewidth">
                    <Heading >Schedule Meeting</Heading>
                        <a
                            href="https://calendly.com/swatishukla/walmart_ca"
                            target="_blank"
                            style={{ cursor: "pointer" }}
                        >
                            <img
                                ...
                                className="img-fluid"
                                id="imageclasscheckfistImage"
                                src={require("../../../../assets/img/meeting_all.gif")}
                                alt="Store Development"
                                /*height="650" width="650"*/
                            />
                        </a>
