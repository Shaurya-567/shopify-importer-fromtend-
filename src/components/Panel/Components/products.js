...
export class Products extends Component {
  ...
  massActions = [{ label: "Upload", value: "upload" },{ label: "Delete", value: "Delete" }];
  ...
  handleeditproductdata(row) {
    ...
    setTimeout(() => {
      this.redirect("/panel/products/editsection/" + row, {
  ...
  handleDeleteSeleteproductdata(data){
      requests.postRequest("frontend/importer/deleteSeleteProduct",data).then((response) => {
       if(response.success){
        notify.success(response.code);
  ...
  handleSelectedUpload = (arg, val) => {
    switch (arg) {
      case "delete":
        let alldata={
          source_product_id:this.state.selectedProducts,
          merchant_id:this.state.merchant_id,
          marketplace:this.filters.marketplace
        }
        confirmAlert({
          title: "Confirm To Delete Seleted This  Product ",
          message: "Are you sure to delete seleted this product",
          buttons: [
            {
              label: "Yes",
              onClick: this.handleDeleteSeleteproductdata.bind(this,alldata),
            },
            {
              label: "No",
              onClick: () => console.log(),
                    ...
                    massAction={(event) => {
                      switch (event) {
                        case "upload":
                          ...
                          case "Delete": 
                          this.state.selectedProducts.length > 0
                            ? this.handleSelectedUpload("delete")
                            : notify.info("No Product Selected");
                          break;
